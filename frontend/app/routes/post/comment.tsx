...
export default function Comment({ comment, onReply }: { comment: CommentType; onReply: () => void }) {
  ...
  return (
    <div key={comment.id} className="mb-4 flex flex-col gap-1">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500">{fromNowDate({ date: comment.created, shorten: true })}</span>
      </div>
      <p className="text-sm py-2 px-1">{comment.content}</p>
